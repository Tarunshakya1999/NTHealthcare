...
const Orders = () => {
  ...
  const [returnOrder, setReturnOrder] = useState(null);   // 🆕 store full order object
  ...
  const daysLeft = (order) => {
    ...
    return Math.max(
      0,
      7 - Math.floor((new Date() - new Date(order.delivered_at)) / (1000 * 60 * 60 * 24))
    );
  };
                  ...
                  <div className="card shadow-sm p-3 rounded-3">
                    <div className="d-flex justify-content-between">
                        ...
                        {order.status === "DELIVERED" && canReturn(order) && (
                          <div className="mt-2">
                            <small className="text-success">
                              {daysLeft(order)} day{daysLeft(order) !== 1 ? "s" : ""}{" "}
                              left to return
                            </small>
                          </div>
                        )}
                      ...
                      <div className="text-end">
                        {/* Return button – opens ReturnRequestModal */}
                        {order.status === "DELIVERED" && canReturn(order) && (
                          <button
                        ...
                        {/* Cancel button – opens CancelOrderModal */}
                        {["PENDING", "VERIFIED", "PROCESSING"].includes(
                          order.status
                        ) && (
                          <button
